...
function clickedCell(cell) {
	...
	if (gameOver(board, COMP)) {
		...
		msg.innerHTML = "Та хожигдлоо!";
	}
	if (emptyCells(board).length == 0 && !gameOverAll(board)) {
		var msg = document.getElementById("message");
		msg.innerHTML = "Тэнцлээ!";
	}
	if (gameOverAll(board) == true || emptyCells(board).length == 0) {
		button.value = "Дахин эхлүүлэх";
		button.disabled = false;
	}
}

function restartBnt(button) {
	if (button.value == "AI ийг асаах") {
		aiTurn();
		button.disabled = true;
	}
	else if (button.value == "Дахин эхлүүлэх") {
		var htmlBoard;
		var msg;
